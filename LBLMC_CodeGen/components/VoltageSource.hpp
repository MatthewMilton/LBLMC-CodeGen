...
#ifndef LBLMC_VOLTAGESOURCE_HPP
#define LBLMC_VOLTAGESOURCE_HPP
...
public:
	...
	VoltageSource(std::string comp_name);
	VoltageSource(std::string comp_name, double v, double res);
	VoltageSource(const VoltageSource& base);
	...
	inline void setParameters(double v, double res) { VOLTAGE = v; RES = res; }
